...
<body>
            ...
            <div class="row">
                <div class="col-lg-12">
                    ...
                    <div class="position-relative mb-3">
                        <img class="img-fluid w-100" src="img/Untitled.jpg" style="object-fit: cover;">
                        <div class="bg-white border border-top-0 p-4" id="noticiaDetails">
                            ...
                            if (isset($_GET['id'])) {
                                ...
                                // Consulta para obter os detalhes da notícia pelo ID
                                $sql = "SELECT * FROM noticias WHERE id = $noticiaId";
                        ...
                        <div class="d-flex justify-content-between bg-white border border-top-0 p-4">
                            <div class="d-flex align-items-center">
                                <span id="author"><?php echo isset($noticia['autor']) ? $noticia['autor'] : ''; ?></span>
                            </div>
                        </div>
                    </div>
                    <!-- News Detail End -->
    ...
    <script>
        // Use JavaScript para fazer uma solicitação AJAX para obter os detalhes da notícia
        document.addEventListener("DOMContentLoaded", function() {
            // Supondo que o ID da notícia seja passado via parâmetro GET
            const noticiaId = <?php echo isset($_GET['id']) ? $_GET['id'] : 'null'; ?>;

            // Verifique se há um ID de notícia válido
            if (noticiaId !== null) {
                const noticiaDetails = document.getElementById('noticiaDetails');
                ...
                xhr.onreadystatechange = function() {
                    if (xhr.readyState === XMLHttpRequest.DONE) {
                        if (xhr.status === 200) {
                            ...
                            // Exiba os detalhes da notícia
                            noticiaDetails.innerHTML = `
                                <h2>${noticia.titulo}</h2>
                                <p>Por: ${noticia.autor}</p>
                                <p>${noticia.texto}</p>
                            `;
                            authorElement.innerText = noticia.autor;
                        } else {
                            console.error('Erro ao carregar detalhes da notícia:', xhr.status);
                        }
                    }
                };
                xhr.open('GET', './PHP/noticias.php?id=' + noticiaId);
                xhr.send();
            }
        });
    </script>
